...
def _es_saturacion(e):
    """Detecta si el error de Gemini es por saturación/cuota (429, 503, etc.)."""
    msg = str(e).lower()
    señales = ["429", "resource_exhausted", "quota", "rate limit",
               "overloaded", "unavailable", "503", "500", "internal"]
    return any(s in msg for s in señales)

def generar_gemini_con_reintentos(gemini_client, model, contents, intentos=3, espera_base=2):
    """Llama a Gemini reintentando con backoff (2, 4, 8 seg) si está saturado."""
    ultimo_error = None
    for i in range(intentos):
        try:
            ...
            ultimo_error = e
            if _es_saturacion(e) and i < intentos - 1:
                time.sleep(espera_base * (2 ** i))  # 2s, 4s, 8s
                continue
            raise
    raise ultimo_error
...
def procesar_venta(gemini_client, co_client, contenido, tipo, perfil=None):
    ...
    try:
        ...
        # Gemini para audio e imagen (con reintentos ante saturación)
        if gemini_client:
            if tipo == "audio":
                r = generar_gemini_con_reintentos(
                    gemini_client, "gemini-2.5-flash",
                    [prompt, "Transcribí el audio y extraé los datos.", contenido]
                )
            elif tipo == "imagen":
                r = generar_gemini_con_reintentos(
                    gemini_client, "gemini-2.5-flash",
                    [prompt, "Analizá la imagen y extraé los datos.", contenido]
                )
            raw = r.text.strip().replace("```json","").replace("```","").strip()
            return json.loads(raw)

    except json.JSONDecodeError:
        st.warning("⚠️ No pudo interpretar el contenido. Intentá reformular o cargá la venta por texto.")
        return None
    except Exception as e:
        if _es_saturacion(e):
            st.warning("🕐 El servicio de audio/imagen está ocupado en este momento. "
                       "Esperá un minuto y volvé a intentar, o cargá la venta por texto mientras tanto.")
        else:
            st.error(f"❌ Error: {e}")
        return None
    ...
    with col2:
        ...
        for i, p in enumerate(productos):
            ...
            with st.expander(f"{icon} {p.get('descripcion','Producto')} — {cant_actual} {p.get('unidad','')} × ${precio_actual:,.0f}", expanded=p.get("fuera_de_rubro", False)):
                ...
                with ec1:
                    p["descripcion"] = st.text_input("Descripción", value=p.get("descripcion",""), key=f"desc_{fuente}_{ik}_{i}")
                with ec2:
...
with tab2:
    ...
    render_resultado_venta("audio", gc, sheets_ok, nro_comprobante)
...
with tab3:
    ...
    render_resultado_venta("imagen", gc, sheets_ok, nro_comprobante)
